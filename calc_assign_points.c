...
FILE * tf1;
    ...
    if((tf1 = fopen("Fen_2019_temp//calc_resalt_temp.txt","a")) == NULL)
    {
    printf("Error creation file calc_resalt_temp.txt\n");
    return 1;
    }
      fprintf (tf1, "%d %.3f %.3f  %.10e %.10e  %.10e\n", n_point_tt, p1_l, p2_l, y[0], y[1], y[2]);
      fclose(tf1);
     }
...
*
*  Параметры импульса поля читаются из текстового файла task_q_easy
...
*  
*  Список точек для счета берется из обновляемого файла q_tree_easy
...
main (int argc, char **argv)
{
    ...
    if (argc < 3) 
    {
      printf("no files task_q_easy or q_tree_easy!\n");
      exit(1);
    }
    ...
    {
    printf("Error opening file task_q_easy\n");
    return 1;
    }
   ...
   double f3_t;
    ...
    char *filename2 = argv[2];
       ...
       if((q_tree = fopen(filename2,"r")) == NULL)
        {
          printf("Error opening file q_tree_easy\n");
          return 1;
        }
